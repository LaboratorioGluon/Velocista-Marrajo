...
#include "drivers/adc_continuous_internal.h"
//For DMA
...
esp_err_t adc_continuous_flush_pool(adc_continuous_handle_t handle)
{
    ...
    ESP_RETURN_ON_FALSE(handle->fsm != ADC_FSM_STARTED, ESP_ERR_INVALID_STATE, ADC_TAG, "ADC continuous mode isn't in the init state, it's started already");
